...
export class AdminCategoriesPage implements OnInit {
  ...
  readonly columns = computed<DataTableColumn<CategoryResponse>[]>(() => {
    this.langChange();

    return [
      {
        key: 'imageUrl',
        header: this.translate.instant('admin.categories.columns.image'),
        type: 'image',
        accessor: r => r.imageUrl || '',
      },
      {
        key: 'name',
        header: this.translate.instant('admin.categories.columns.englishName'),
      },
      {
        key: 'arabicName',
        header: this.translate.instant('admin.categories.columns.arabicName'),
      },
      {
        key: 'isPublished',
        header: this.translate.instant('admin.categories.columns.published'),
        type: 'toggle',
        align: 'center'
      },
      {
        key: 'description',
        header: this.translate.instant('admin.categories.columns.description'),
      ...
      this.adminCatalogService.updateCategory(this.editingCategoryId()!, updateData, this.selectedImageFile).subscribe({
        next: (res) => {
          ...
          if (res.success) {
            ...
            this.formError.set(res.message || this.translate.instant('admin.categories.updateFailed'));
          }
        },
        error: (err) => {
          this.isSubmitting.set(false);
          this.formError.set(extractErrorMessage(err, this.translate.instant('admin.categories.updateFailed')));
        },
      });
    } else {
      ...
      this.adminCatalogService.createCategory(createData, this.selectedImageFile).subscribe({
        next: (res) => {
          ...
          if (res.success) {
            ...
            this.formError.set(res.message || this.translate.instant('admin.categories.createFailed'));
          }
        },
        error: (err) => {
          this.isSubmitting.set(false);
          this.formError.set(extractErrorMessage(err, this.translate.instant('admin.categories.createFailed')));
        },
      });
    }
  }
  ...
  deleteCategory(category: CategoryResponse): void {
    const message = this.translate.instant('admin.categories.confirmDelete', { name: category.name });
    ...
    this.adminCatalogService.deleteCategory(category.id).subscribe({
      next: (res) => {
        ...
        if (res.success) {
          ...
          this.deleteError.set(res.message || this.translate.instant('admin.categories.deleteFailed'));
        }
      },
      error: (err) => {
        this.isDeletingId.set(null);
        this.deleteError.set(extractErrorMessage(err, this.translate.instant('admin.categories.deleteFailed')));
      },
    });
  }
  ...
  onRowClick(category: CategoryResponse): void {
